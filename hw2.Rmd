...
After creating the dataframe, we used `dplyr` to manipulate the dataframe. We first use `select()` to keep just first and last names, then use `distinct ()` to keep only the 250 unique names in this case. Then, we use the `count()` and `sort()` function to get the most common first/last name at the top row (ties broken by alphabetical order).
```{r}
l_name = lego_df %>%
