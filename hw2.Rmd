...
* Next identify we want the subcatagory names for each catagory name. In this case, only purchases has a subcatagory name. Thankfully our nested list only two deep (no subsubcatagory names). We use a similar technique to the top level names except we want to know the length.
...
After creating the dataframe, we can use *dplyr* to manipulate the dataframe. We first use *select()* function to first select first name and last name from the lego dataframe, then use *distinct ()* to delete all the duplicates names and only keep unique names, which are 250 names in this case. Then, we use the *count()* and *sort()* function to get the most commonly appeared first/last name at the top row, and least common appeared name at the bottom, with their number of occurences too. Finally, we print out the first row, which is the most common fist/last name of purchasers. We found out that **Jacob** and **Brown** are the most common first and last names, respectively. 
```{r}
lego_df %>%
...
For the easy way, we looping over each individual's hobbies in sales data, and we can get a table of everyone's hobbies. After manually sorting we found that slicing the top 12 rows will give us the result. Note that all hobbies tied No.4 are considered among the five most popular hobbies. 

For the harder way that uses dplyr, we first use sapply to put all of the hobbies from all columns into one vector, hobbies.We build the dataframe for hobbies, and use *count()* to count the number of hobbies occured. Then we use *arrange(desc())* to sort the dataframe in decending order, and use slicefunction to get the top 5 hobbies. Finally, use *select()* to only print out the top 5 hobbies without their number of occurence. In this case, there are 8 same number of occurences (occured 4 times) for the fifth popular hobby, however, we only use slice(1:5), so we only get first hobby of 4 times occured hobbies by the order of alphabet. So, the most popular hobbies we get are **Animal fancy, Skateboarding, Kitesurfing, Sculling or Rowing and Baseball**.
```{r}
...
Again we use dplyr to manipulate the dataset. We select pieces and quantity from dataframe and use *transmute()* to make new columns of *Total_Prices*, replacing existing columns of *Pieces* and *Quantity*. We sum up the rows in the newly created column and get a single number of the total purchased pieces, which is **202351**.
...
First we create a list of US area codes after some research. We convert the area codes in lego_df from characters to numeric values by using substr(). Then we create a function *state()* that checks to which state each area code of the phone number is associated; the function will return NA if the area code is unindentified as a state. We add the area codes and their corresponding states to the lego_df data frame.
...
Our answer to the question is that **Texas** spent the most money on legos.
```{r}
