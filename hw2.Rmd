...
After creating the dataframe, we used `dplyr` to manipulate the dataframe. We first use `select()` to keep just first and last names, then use `distinct ()` to keep only the 250 unique names in this case. Then, we use the `count()` and `sort()` function to get the most common first/last name at the top row (ties broken by alphabetical order). We found out that **Jacob** and **Brown** are the most common first and last names, respectively. 
```{r}
lego_df %>%
...
Again we use dplyr to manipulate the dataset. We use `select()` to first select `Name` and `Quatinty` from the lego dataframe. Multiple people can purchase the same lego set. Therefore need to group by name of the lego set and then sum up the quantity purchased by different customers. We arrange the summary table in a descending order by quantity and use `slice()` to select the top 5 rows (once again ties broken by alphabetical order of names). **Campsite Scrapbook, Ninjago Party Set, Arctic, Daisy's Beauty Salon and Finn** are the five most popular lego sets we found.
```{r}
lego_df %>%
...
We again use dplyr on the dataframe. We use `mutate()` to create a new variable `Spent` as a product of price and quantity to get total amount of money spent. Using the similar idea in **2)**, we arrange `Spent` in a descending order and slice the top 5 rows to get the results. Finally, we drop the `USPrice` and `Quantity` columns. Our answer to the question is that **Jennifer Dana, Cecilia Mauro, Kaitlyn Degeer, Elias Garret and Hector Collins** have spent the most money, and they spent **699.98, 359.97, 339.98, 269.97 and 249.99** accordingly.
...
###5. Do men or women buy more lego sets (per person) on average?
  ...
  Grouping by gender and finding the mean of `Quantity` we found that **male** on average buy slightly more lego sets than **female**. However the difference is almost negligible and more tests should be conducted.
```{r}
lego_df %>%
...
We looped through the sales data frame with sapply and extracted hobbies from each person. We build the dataframe for hobbies, and use `count()` to count the number of hobbies occured. Then we use `arrange(desc())` to sort the dataframe in decending order, and use `slice` function to get the top 5 hobbies (ties broken by alphabetical order). So, the most popular hobbies we get are **Animal fancy, Skateboarding, Kitesurfing, Sculling or Rowing and Baseball**.
```{r}
...
Again we use dplyr to manipulate the dataset. We select pieces and quantity from dataframe and use `transmute()` to make new columns of `Total_Prices`, and dropping the existing columns of `Pieces` and `Quantity`. We sum up the rows in the newly created column and get a single number of the total purchased pieces, which is **202351**.
