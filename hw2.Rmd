...
=======
>>>>>>> 2d5d4b4689bead84d27ea40328be04ed63ebeacf
...
First we create a list of US area codes after some research. We convert the area codes in lego_df from characters to numeric values Then we create a function *state()* that checks to which state each area code of the phone number is associated; the function will return NA if the area code is unindentified as a state. 

Finally we use the same logic as shown in *7)* that select relevant variables, groups by state and returns the summary of the row values from the transmuted column state. 
```{r}
